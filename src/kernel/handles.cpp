...
void Handles::init_console_handles(){
	...
	handles.insert(std::pair<kiv_os::THandle, std::shared_ptr<Handler>>(kiv_os::stdInput, conIn));

	std::shared_ptr<Console> conOut = std::make_shared<Console>(kiv_os::stdOutput);
	handles.insert(std::pair<kiv_os::THandle, std::shared_ptr<Handler>>(kiv_os::stdOutput, conOut));

	std::shared_ptr<Console> conError = std::make_shared<Console>(kiv_os::stdError);
	handles.insert(std::pair<kiv_os::THandle, std::shared_ptr<Handler>>(kiv_os::stdError, conError));
}
...
kiv_os::THandle Handles::add_handle(std::shared_ptr<Handler> hnd) {
	...
	handles.insert(std::pair<kiv_os::THandle, std::shared_ptr<Handler>>(Last_Handle, hnd));
	return Last_Handle;
}
